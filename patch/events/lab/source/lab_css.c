...
void Menu_SelCard_Think(GOBJ *menu_gobj) {
    ...
    // cursor movement
    if (down & (HSD_BUTTON_LEFT | HSD_BUTTON_DPAD_LEFT)) {
                            ...
                            if (CARDGetStatus(slot, file_no, &card_stat) == CARD_RESULT_READY) {
                                ...
                                if (CARDOpen(slot, file_name, &card_file_info) == CARD_RESULT_READY) {
                                    ...
                                    if (CARDRead(&card_file_info, buffer, CARD_READ_SIZE, 0x1E00) ==
                                        ...
                                        // get to header (need to find a less hardcoded way of doing this)
        ...
        case (CFRM_LOAD): {
            ...
            // check for select
                ...
                if (cursor == 0) {
                    ...
                    css_minorscene->vs_data.match_init.playerData[hmn_index].costume = hmn_costume;
                    ...
                    css_minorscene->vs_data.match_init.playerData[cpu_index].costume = cpu_costume;
