...
static char **LabValues_InfoDisplay[] = {
    ...
    "Engine LStick", "System LStick", "Engine CStick", "System CStick", "Engine Trigger", "System Trigger",
    "Ledgegrab Timer", "Intangibility Timer", "Hitlag", "Hitstun", "Shield Health", "Shield Stun", "Grip Strength",
...
static EventOption LabOptions_Record[] = {
    ...
    },
    {
        ...
        .desc =
        "Toggle which recording slot to save inputs \nto. Maximum of 6 and can be set to random \nduring playback.",
        // string describing what this option does
...
static CPUAction *Lab_CPUActions[] = {
    0, // none 0
    Lab_CPUActionShield, // shield 1
    Lab_CPUActionGrab, // grab 2
    Lab_CPUActionUpB, // up b 3
    Lab_CPUActionDownB, // down b 4
    Lab_CPUActionSpotdodge, // spotdodge 5
    Lab_CPUActionRollAway, // roll away 6
    Lab_CPUActionRollTowards, // roll towards 7
    Lab_CPUActionRollRandom, // roll random
    Lab_CPUActionNair, // nair 8
    Lab_CPUActionFair, // fair 9
    Lab_CPUActionDair, // dair 10
    Lab_CPUActionBair, // bair 11
    Lab_CPUActionUair, // uair 12
    Lab_CPUActionJump, // short hop 13
    Lab_CPUActionJumpFull, // full hop 14
    Lab_CPUActionJumpAway, // jump away 15
    Lab_CPUActionJumpTowards, // jump towards 16
    Lab_CPUActionAirdodge, // airdodge 17
    Lab_CPUActionFFTumble, // fastfall 18
    Lab_CPUActionFFWiggle, // wiggle fastfall 19
    Lab_CPUActionJab, // wiggle fastfall 19
    Lab_CPUActionFTilt,
    Lab_CPUActionUTilt,
    Lab_CPUActionDTilt,
    Lab_CPUActionUSmash,
...
static u8 stc_hmn_controller; // making this static so importing recording doesn't overwrite
static u8 stc_cpu_controller; // making this static so importing recording doesn't overwrite
...
void Lab_ChangeCamMode(GOBJ *menu_gobj, int value) {
    MatchCamera *cam = (MatchCamera *)MATCH_CAM;
...
GOBJ *InfoDisplay_Init() {
    ...
    JOBJ *corners[4];
    JOBJ_GetChild(menu, (int)&corners, 2, 3, 4, 5, -1);
...
void InfoDisplay_Think(GOBJ *gobj) {
    ...
    if (Pause_CheckStatus(1) != 2) {
        ...
        if (rowsEnabled != 0) {
            ...
            FighterData *fighter_data = (FighterData *)-1;
            if (fighter != 0) {
                ...
                if (idOptions[i + OPTINF_ROW1].option_val == 0 || fighter == 0) {
                    ...
                    switch (value) {
                        ...
                        case INFDISPROW_FRAME: {
                            float *animStruct = (float *)fighter_data->anim_curr_flags_ptr;
                            int frameCurr = 0;
                            int frameTotal = 0;
                        ...
                        case INFDISPROW_SELFVEL: {
                            Text_SetText(text, i, "SelfVel: (%+.3f , %+.3f)", fighter_data->phys.self_vel.X,
                                         fighter_data->phys.self_vel.Y);
                            break;
                        }
                        case INFDISPROW_KBVEL: {
                            Text_SetText(text, i, "KBVel: (%+.3f , %+.3f)", fighter_data->phys.kb_vel.X,
                                         fighter_data->phys.kb_vel.Y);
                            break;
                        }
                        case INFDISPROW_TOTALVEL: {
                            Text_SetText(text, i, "TotalVel: (%+.3f , %+.3f)",
                                         fighter_data->phys.self_vel.X + fighter_data->phys.kb_vel.X,
                                         fighter_data->phys.self_vel.Y + fighter_data->phys.kb_vel.Y);
                            break;
                        }
                        ...
                        case INFDISPROW_SYSTRIGGER: {
                            HSD_Pad *pad = PadGet(ply, PADGET_MASTER);
                            Text_SetText(text, i, "Trigger Sys: (%+.3f , %+.3f)", pad->ftriggerLeft,
                                         pad->ftriggerRight);
                            break;
                        }
                        ...
                        case INFDISPROW_JUMPS: {
                            Text_SetText(text, i, "Jumps: %d/%d", fighter_data->jump.jumps_used,
                                         fighter_data->attr.max_jumps);
                            break;
                        }
...
int CPU_IsThrown(GOBJ *cpu) {
    ...
    // check if thrown
    if ((cpu_state >= ASID_THROWNF && cpu_state <= ASID_THROWNLW) || cpu_state == ASID_CAPTURECAPTAIN || cpu_state == ASID_THROWNKOOPAF || cpu_state == ASID_THROWNKOOPAB || cpu_state == ASID_THROWNKOOPAAIRF
...
int CPU_IsGrabbed(GOBJ *cpu) {
    ...
    // check if thrown
    if (cpu_state == ASID_CAPTUREWAITHI || cpu_state == ASID_CAPTUREWAITLW || cpu_state == ASID_CAPTUREWAITKOOPA
        || cpu_state == ASID_CAPTUREWAITKOOPAAIR || cpu_state == ASID_CAPTUREWAITKIRBY || cpu_state == ASID_DAMAGEICE || cpu_state == ASID_CAPTUREMASTERHAND || cpu_state == ASID_YOSHIEGG || cpu_state == ASID_CAPTUREKIRBYYOSHI || cpu_state == ASID_KIRBYYOSHIEGG || cpu_state == ASID_CAPTURELEADEAD || cpu_state == ASID_CAPTURELIKELIKE || cpu_state == ASID_CAPTUREWAITCRAZYHAND || (
            ...
            if (fighter_data->flags.hitlag == 1 && fighter_data->flags.hitstun == 1) {
                ...
                for (int i = 0; i < hitstun_frames; i++) {
                    ...
                    if (vertices_num >= DI_MaxColl) {
                        ...
                        // check for slide off
                        // not touching ground this frame, was touching last frame
                        if ((ecb.envFlags & ECB_GROUND) == 0 && (ecb.envFlags_prev & ECB_GROUND) != 0) {
                ...
                didraw->vertices[ply][0].Y = fighter_data->coll_data.topN_Curr.Y + fighter_data->coll_data.ecbCurr_left.
                                             Y;
                didraw->vertices[ply][1].X = fighter_data->coll_data.topN_Curr.X + asdi_orig.X;
                didraw->vertices[ply][1].Y = fighter_data->coll_data.topN_Curr.Y + fighter_data->coll_data.ecbCurr_left.
                                             Y + asdi_orig.Y;

                // save this info to the draw struct
                for (int i = 0; i < vertices_num; i++) {
                    didraw->vertices[ply][i + 2].X = DICollData[i].pos.X;
                    didraw->vertices[ply][i + 2].Y =
                            DICollData[i].pos.Y + (DICollData[i].ECBLeftY + DICollData[i].ECBTopY) / 2;
        ...
        if (fighter != 0) {
            ...
            if (stickX != 0 || stickY != 0) {
                ...
                if ((held & HSD_BUTTON_A) != 0) {
                    ...
                    // adjust rotate
                    MatchCamera *matchCam = (MatchCamera *)MATCH_CAM;
...
void Inputs_Think(GOBJ *gobj) {
    ...
    for (int i = 0; i < sizeof(input_data->controller_joint) / 4; i++) {
        ...
        if (controller != 0) {
            ...
            // move lstick
            JOBJ *lstick_joint;
            JOBJ_GetChild(controller, (int)&lstick_joint, 10, -1);
            lstick_joint->trans.X = pad->fstickX * 2.3;
            lstick_joint->trans.Y = pad->fstickY * 2.3;

            // move lstick
            JOBJ *rstick_joint;
            JOBJ_GetChild(controller, (int)&rstick_joint, 8, -1);
            rstick_joint->trans.X = pad->fsubstickX * 2.3;
            rstick_joint->trans.Y = pad->fsubstickY * 2.3;

            // move ltrigger
            JOBJ *ltrig_joint;
            JOBJ_GetChild(controller, (int)&ltrig_joint, button_lookup[BTN_L].jobj, -1);
            ...
            // move rtrigger
            JOBJ *rtrig_joint;
            JOBJ_GetChild(controller, (int)&rtrig_joint, button_lookup[BTN_R].jobj, -1);
            ...
            for (int j = 0; j < BTN_NUM; j++) {
                ...
                JOBJ *button_jobj;
                JOBJ_GetChild(controller, (int)&button_jobj, button_lookup[j].jobj, -1);
                DOBJ *button_dobj = JOBJ_GetDObjChild(button_jobj, button_lookup[j].dobj);

                if (held & button_bits[j]) {
                    ...
                    GXColor *color_released = &button_colors[j];
...
void Inputs_Init() {
    ...
    for (int i = 0; i < hmn_count; i++) {
        ...
        if (found_origin == 0) {
            ...
            JOBJ_GetChild(controller, (int)&ltrig_jobj, button_lookup[BTN_L].jobj, -1);
            JOBJ_GetChild(controller, (int)&rtrig_jobj, button_lookup[BTN_R].jobj, -1);
...
GOBJ *Record_Init() {
    ...
    // save seek jobj
    JOBJ *seek;
    JOBJ_GetChild(playback, (int)&seek, REC_SEEKJOINT, -1);
    rec_data.seek_jobj = seek;

    // save left and right seek bounds
    JOBJ *seek_bound[2];
    JOBJ_GetChild(playback, (int)&seek_bound, REC_LEFTBOUNDJOINT, REC_RIGHTBOUNDJOINT, -1);
    Vec3 seek_bound_pos;
    ...
    // Get text positions
    JOBJ *text_joint[2];
    JOBJ_GetChild(playback, (int)&text_joint, REC_LEFTTEXTJOINT, REC_RIGHTTEXTJOINT, -1);
                            ...
                            if (strncmp(os_info->gameName, card_stat.gameName, sizeof(os_info->gameName)) == 0) {
                                ...
                                if (strncmp("TMREC", card_stat.fileName, 5) == 0) {
                                    ...
                                    if (tmrec_num == file_no) {
                                        file_found = 1;
                                        memcpy(&filename, card_stat.fileName, sizeof(filename));
                                        // copy filename to load after this
    ...
    if (Pause_CheckStatus(1) != 2) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (fighter != 0) {
                ...
                // check for savestate
                int blacklist = HSD_BUTTON_DPAD_DOWN | HSD_BUTTON_DPAD_UP | HSD_TRIGGER_Z | HSD_TRIGGER_R | HSD_BUTTON_A |
...
void Export_Init(GOBJ *menu_gobj) {
    ...
    // save jobj pointers
    JOBJ_GetChild(export_joint, (int)&export_data->memcard_jobj[0], EXP_MEMCARDAJOBJ, -1);
    JOBJ_GetChild(export_joint, (int)&export_data->memcard_jobj[1], EXP_MEMCARDBJOBJ, -1);
    JOBJ_GetChild(export_joint, (int)&export_data->screenshot_jobj, EXP_SCREENSHOTJOBJ, -1);
    JOBJ_GetChild(export_joint, (int)&export_data->textbox_jobj, EXP_TEXTBOXJOBJ, -1);
...
int Export_Process(GOBJ *export_gobj) {
    ...
    // if snapshot is processing, don't update
...
void Event_Init(GOBJ *gobj) {
    ...
    HSD_Update *hsd_update = (HSD_Update *)HSD_UPDATE;
    ...
    if (pad->down == PAD_BUTTON_DPAD_DOWN) {
        ...
        if (cpu_data->flags.dead == 0) {
            ...
            if (hmn_data->phys.air_state == 0) {
                ...
                float toY = fromY - 10;
                isGround = GrColl_RaycastGround(&coll_pos, &line_index, &line_kind, &line_unk, (Vec3 *)-1, (Vec3 *)-1, (Vec3 *)-1, 0, fromX, fromY, toX, toY, 0);
                if (isGround == 1) {
